...
if ($type === 'final') {
    ...
    // Aynı adayın (burada: aynı oturumun) tek yazarı konuşmacı olduğu için
    // eşzamanlı yazma yarışı pratikte yok - basit oku-yaz-artır yeterli
    // (bkz. apps/translate Cloudflare sürümündeki Durable Object seq mantığı).
    ...
    // Çeviri BURADA yapılmıyor (WebSocket sürümünden FARKLI) - kimin hangi
    // dili dinlediğini bu istek anında bilmiyoruz (polling modeli, kalıcı
    // bağlantı yok). Çeviri, participant_poll.php'de İLK istendiğinde
    // tembel (lazy) olarak yapılıp önbelleğe alınıyor - aynı dile ihtiyaç
    // duyan sıradaki katılımcı/anket için tekrar çağrılmıyor.
